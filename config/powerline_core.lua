...
function addArrowEnd(text, oldColor)
	-- [0m at the end turns off all attributes
	--text = text..ansiEscChar.."["..oldColor.foreground.."m"..plc_prompt_arrowSymbol..ansiEscChar.."[0m"
	text = text..ansiEscChar.."["..oldColor.foreground..";"..plc_prompt_arrowSymbol..""..ansiEscChar.."[0m"
	return text
end 
...
function addSegment(text, textColor, fillColor) 	
	...
	if currentSegments == "" then 
		...
		newPrompt = addArrow(currentSegments, currentFillColor, fillColor)
	end 
	...
	--newPrompt = addArrowEnd(newPrompt, fillColor)
